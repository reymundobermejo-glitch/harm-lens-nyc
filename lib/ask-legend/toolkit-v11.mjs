...
export const WALK_THROUGH_STEPS = Object.freeze([
  { tab: "why", caption: "Counts — Hurt and Died, and whether this still shows if we change the window." },
...
const INSPECT_TABS = new Set(["why", "records", "situate", "packet"]);
...
export function walkThroughPlace(raw = {}, session = {}) {
  ...
  return {
    ...
    effect: "Walk Counts → Crashes → On the street",
  };
}
...
export function parseCloserLookJob(utterance, sessionState = {}, ctx = {}) {
  ...
  if (/walk me through this place/.test(key) || /^walk me through\b/.test(key)) {
    ...
    return jobOk("investigate", "Walk through Counts, Crashes, then On the street", [
